...
int guild_lookup (const char *name)
{
   ...
   for ( guild = 0; guild_table[guild].name != NULL; guild++)
   {
	if (LOWER(name[0]) == LOWER(guild_table[guild].name[0])
	&&  !str_prefix( name,guild_table[guild].name))
	    return guild;
   }
...
void reset_char(CHAR_DATA *ch)
{
	...
	ch->perm_hit 	= ch->real_hit;
	ch->perm_mana 	= ch->real_mana;
	ch->perm_move	= ch->real_move;
    ...
    ch->max_hit		= ch->real_hit;
    ch->max_mana	= ch->real_mana;
    ch->max_move	= ch->real_move;
...
void affect_get(CHAR_DATA *ch, int sn, AFFECT_DATA *af)
{
    AFFECT_DATA *paf;
...
void char_from_room( CHAR_DATA *ch )
{
    ...
    if (!IS_NPC(ch) && ch->pcdata->locker!=NULL)
	do_storage(ch,"Close");
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    &&   (get_trust(ch) < victim->incog_level)
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if ( (obj->item_type == ITEM_TRAP) && (!IS_AFFECTED(ch,AFF_DETECT_TRAPS)) )
	return FALSE;
...
char *affect_bit_name( int vector )
{
    ...
    if ( vector & AFF_CHARISMA      ) strcat( buf, " charisma"       );
    if ( vector & AFF_SANCTUARY     ) strcat( buf, " sanctuary"     );
    if ( vector & AFF_FAERIE_FIRE   ) strcat( buf, " faerie_fire"   );
    if ( vector & AFF_INFRARED      ) strcat( buf, " infrared"      );
    if ( vector & AFF_CURSE         ) strcat( buf, " curse"         );
    if ( vector & AFF_POISON        ) strcat( buf, " poison"        );
    if ( vector & AFF_PROTECT       ) strcat( buf, " protect evil" );
    if ( vector & AFF_PROTECT_GOOD  ) strcat( buf, " protect good"  );
    if ( vector & AFF_DISGUISE      ) strcat( buf, " disguise"     );
...
char *comm_bit_name(int comm_flags)
{
    ...
    if (comm_flags & COMM_NONEW 	) strcat(buf, " no_new");
    ...
    if (comm_flags & COMM_VT100		) strcat(buf, " vt100_on");
