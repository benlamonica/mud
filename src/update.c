...
/*    add_hp = add_hp * 9/10;
	...
	ch->real_hit += add_hp;
	ch->real_mana += add_mana;
	ch->real_move += add_move;
    ch->max_hit 	+= add_hp;
...
void gain_exp( CHAR_DATA *ch, int gain )
{
    ...
    ch->exp = UMAX( exp_per_level(ch,ch->points), ch->exp + gain );
    while ( ch->level < LEVEL_HERO && ch->exp >= 
	exp_per_level(ch,ch->points) * (ch->level+1) )
    {
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	{
	case COND_FULL:
	    if (ch->desc && ch->desc->connected == CON_PLAYING && !IS_SET(ch->comm,COMM_AFK))
...
{   
	...
	if( !IS_NPC(ch) && ((ch->pk_timer > 0 || !IS_SET(ch->in_room->room_flags, ROOM_SAFE))) && ch->pcdata->virtual_level[1]>0)
        {
		ch->pcdata->virtual_level[1]--;
		if(ch->pcdata->virtual_level[1]==0)
...
/*
	...
	case ITEM_FURNITURE:  message = "The ground opens up and swallows $p."; break;
	case ITEM_PORTAL:     message = "$p grows unstable and vanishes from view."; break;
	case ITEM_TRAP:	      message = "$p disappears from whence it came."; break;
	}
...
void aggr_update( void )
{
	...
	for ( ch = wch->in_room->people; ch != NULL; ch = ch_next )
	{
	    ...
	    if(IS_AFFECTED(victim, AFF_CHARISMA) && (victim->level+30 >= ch->level))
		continue;
...
/*We don't need the ticks in the log everytime there is one.*/
