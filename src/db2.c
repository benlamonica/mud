...
/*
 * Snarf a mob section.  new 2.4 style
 */
void load_mobiles24( FILE *fp )
{
    MOB_INDEX_DATA *pMobIndex;
    char buf[100];
...
void load_objects24( FILE *fp )
{
    ...
    for ( ; ; )
    {
        ...
        if ( letter != '#' )
        {
            bug( "Load_objects24: # not found.", 0 );
            exit( 1 );
        }
	...
	sprintf(buf,"ObjVnum: %d",vnum);
	log_string(buf);

        fBootDb = FALSE;
        if ( get_obj_index( vnum ) != NULL )
        {
            bug( "Load_objects24: vnum %d duplicated.", vnum );
            exit( 1 );
        }
        fBootDb = TRUE;
...
//                  paf->where          = TO_VULN;
                    break;
                default:
                    bug( "Load_objects24: Bad where on flag set.", 0 );
                   exit( 1 );
                }
//              paf->type               = -1;
//              paf->level              = pObjIndex->level;
