...
#define CON_GET_BREATH_SPELL		21
#define CON_REMORT			22
#define CON_GEN_GROUPS_REMORT		23
...
struct pc_race_type  /* additional data for pc races */
{
    ...
    bool        classes[4];		/* determines if they can be in
					   that class*/
...
#define ACT_REPAIRMAN		(D)		/* Repair man.  	*/
...
#define ACT_BANKER		(W)
#define ACT_NO_QUEST            (X)
#define ACT_QUESTMASTER		(Y)
...
#define AFF_CURSE		(K)
#define AFF_PROTECT_GOOD	(L)		
#define AFF_POISON		(M)
#define AFF_PROTECT		(N)
#define AFF_DISGUISE		(O)
#define AFF_SNEAK		(P)
...
#define OBJ_VNUM_PORTAL		     29
#define OBJ_VNUM_GATE		     46
#define OBJ_VNUM_SIGN		     44

#define OBJ_VNUM_MUSHROOM	     20
#define OBJ_VNUM_LIGHT_BALL	     21
#define OBJ_VNUM_SPRING		     22
#define OBJ_VNUM_TRAP		     50
...
#define GATE_BUGGY	3
#define GATE_RANDOM	2
#define GATE_NORMAL_EXIT 1
#define GATE_NOCURSE	4
#define GATE_GOWITH	5
...
#define ROOM_LAW		(S)
#define ROOM_NOGATE		(T)
#define ROOM_NOPORTAL		(U)
#define ROOM_NONEXUS		(V)
...
#define PLR_IMMQUEST		(T)
#define IS_QUESTOR		(IS_SET((ch)->act, PLR_QUESTOR))
...
struct	char_data
{
    ...
    int			dracspell;
    int			screen_height;
    bool		new_char;  //take this out after releveling is done
    MOBPROG_DATA	*mobprog;
};
...
struct	pc_data
{
    ...
    /* last note for the boards */
    NOTE_DATA *		in_progress;
    OBJ_DATA		*locker;
    int			virtual_level[2]; //virtual_level[0] = level, virtual_level[1] = time that you are this level
};
...
extern  const   struct item_lookup_type item_table [];
extern	const	struct	attack_type	attack_table	[];
extern  const	struct  disguise_type	disguise_table	[];
extern	const	struct	guild_type	guild_table	[];
...
void	do_question	args( ( CHAR_DATA *ch, char *argument ) );
...
/* act_obj.c */
void do_storage		args( (CHAR_DATA *ch, char *argument) );
...
void	write_to_buffer	args( ( DESCRIPTOR_DATA *d, const char *txt,
			    int length ) );
void	write_to_buffer1	args( ( DESCRIPTOR_DATA *d, const char *txt,
			    int length ) );
void	write_to_buffer2 args( ( DESCRIPTOR_DATA *d, const char *txt,
			    int length ) );
...
bool	damage		args( ( CHAR_DATA *ch, CHAR_DATA *victim, int dam,
			    int dt, int class ) );
bool	damage2		args( ( CHAR_DATA *ch, CHAR_DATA *victim, int dam,
			    int dt, int class ) );
void	update_pos	args( ( CHAR_DATA *victim ) );
void	stop_fighting	args( ( CHAR_DATA *ch, bool fBoth ) );

/* handler.c */
void affect_get  args( (CHAR_DATA *ch, int sn, AFFECT_DATA *af) );
...
char *	parse_string	args( ( char *argument, char *arg_first ) );
...
/* save.c */
bool	load_locker      args( ( CHAR_DATA *ch ) );
void	save_char_locker args( ( CHAR_DATA *ch ) );
void	save_char_obj	 args( ( CHAR_DATA *ch ) );
bool	load_char_obj	 args( ( DESCRIPTOR_DATA *d, char *name ) );

/* skills.c */
bool 	parse_gen_groups args( ( CHAR_DATA *ch,char *argument ) );
bool 	parse_gen_groups_remort args( ( CHAR_DATA *ch,char *argument ) );
void 	list_group_costs args( ( CHAR_DATA *ch ) );
void 	list_group_costs_remort args( ( CHAR_DATA *ch ) );
