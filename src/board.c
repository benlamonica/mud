...
#define L_IMM (MAX_LEVEL - 9) /* if not already defined */

BOARD_DATA boards[MAX_BOARD] =
{

{ "General",  	"General discussion",            0,     2,     "all", DEF_INCLUDE,10, NULL, FALSE },
...
{ "Guild", 	"Guild messages",		 0,     1,     "all", DEF_EXCLUDE,28, NULL, FALSE },
{ "Personal", 	"Personal messages",		 0,     1,     "all", DEF_EXCLUDE,28, NULL, FALSE },
{ "OOC", 	"Out of Character messages",	 0,     1,     "all", DEF_NORMAL,10, NULL, FALSE },
{ "Immortal",	"Immortal Notes",		 L_IMM,     L_IMM, "imm", DEF_NORMAL, 60, NULL, FALSE },
{ "IMP",	"Imp Notes",			 100,   100,   "imp", DEF_NORMAL, 10, NULL, FALSE}


};

/* The prompt that the character is given after finishing a note with ~ or END */
const char * szFinishPrompt = "(^BWC^NW)ontinue, (^BWV^NW)iew, (^BWP^NW)ost or (^BWF^NW)orget it?";
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
	...
	if ((get_trust(ch) == MAX_LEVEL) && (
        ...
        if (ch->pcdata->guild == guild_lookup(note->to_list) && ch->pcdata->guild!=0)
		return TRUE;
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
	...
	sprintf (buf, "You are now %s a new note on the ^BW%s^NW board.\n\r"
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		for (i = 0; i < MAX_BOARD; i++)
		{
			...
			{ 
				sprintf (buf, "%2d> %12s [%s%4d^NW] %s\n\r", 
				                   count, boards[i].short_name, unread ? "^NR" : "^BG", 
