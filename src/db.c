...
void    load_rom24_area args( ( FILE *fp ) );
void    load_area24     args( ( FILE *fp ) );
void    load_mobiles24  args( ( FILE *fp ) );
void    load_objects24  args( ( FILE *fp ) );
void    load_rooms24    args( ( FILE *fp ) );
...
void boot_db( void )
{
	...
	for ( ; ; )
	{
		...
		word = fread_word( fpArea );
		     ...
		     if ( word[0] == '$'               )                 break;
		else if ( !str_cmp( word, "ROM24"    ) )
		{
		    load_rom24_area(fpArea);
		    break;
                }
...
/*void load_mobprogs( FILE *fp )
{
	...
	{
	    bug( "Load_mobprogs: Mobile %d not found.", vnum );
	    exit( 1 );
	}
	fBootDb = TRUE;

	mobile->mobprog			= NULL;
	mobile->mobprog->vnum		= vnum;
	mobile->mobprog->variables	= NULL;
	...
	for(;;)
	    ...
	    {
		mobile->mobprog->variables = perm_alloc
		word = fread_word(fp);
		if(!str_cmp(
}
*/
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	mob->real_hit = mob->max_hit;
	mob->real_mana = mob->max_mana;
	mob->real_move = mob->max_move;
...
void clone_mobile(CHAR_DATA *parent, CHAR_DATA *clone)
{
    ...
    clone->real_hit = parent->max_hit;
    clone->real_mana = parent->max_mana;
    clone->real_move = parent->max_move;
    clone->balance	= 0;
...
void load_rom24_area( FILE *fpArea )
{
    /*
     ...
     */
            for ( ; ; )
            {
                ...
                word = fread_word( fpArea );
                     ...
                     if ( word[0] == '$'               ) break;
                else if ( !str_cmp( word, "AREA"     ) ) load_area24(fpArea);
                else if ( !str_cmp( word, "HELPS"    ) ) load_helps(fpArea);
                else if ( !str_cmp( word, "MOBOLD"   ) ) load_old_mob(fpArea);
                else if ( !str_cmp( word, "MOBILES"  ) ) load_mobiles24(fpArea);
		else if ( !str_cmp( word, "OBJOLD"   ) ) load_old_obj(fpArea);
                else if ( !str_cmp( word, "OBJECTS"  ) ) load_objects24(fpArea);
                else if ( !str_cmp( word, "RESETS"   ) ) load_resets(fpArea);
                else if ( !str_cmp( word, "ROOMS"    ) ) load_rooms24(fpArea);
                else if ( !str_cmp( word, "SHOPS"    ) ) load_shops(fpArea);
                else if ( !str_cmp( word, "SOCIALS"  ) ) load_socials(fpArea);
                else if ( !str_cmp( word, "SPECIALS" ) ) load_specials(fpArea);
...
void load_area24( FILE *fp )
{
    ...
    pArea->reset_first  = NULL;
    pArea->reset_last   = NULL;
//  pArea->file_name    = ***Removed - BAL***
                          fread_string( fp );
//    pArea->credits         = 
			  fread_string( fp );
    pArea->name         = fread_string( fp );
//  pArea->min_vnum     = ***Removed - BAL***
                          fread_number(fp);
//  pArea->max_vnum     = ***Removed - BAL***
                          fread_number(fp);
    pArea->age          = 15;
    pArea->nplayer      = 0;
    pArea->empty        = FALSE;
...
void load_rooms24( FILE *fp )
{
    ...
    if ( area_last == NULL )
    {
        bug( "Load_resets24: no #AREA seen yet.", 0 );
        exit( 1 );
    }
	...
	sprintf(buf,"RoomVnum: %d",vnum);
	log_string(buf);

        fBootDb = FALSE;
        if ( get_room_index( vnum ) != NULL )
        {
            bug( "Load_rooms24: vnum %d duplicated.", vnum );
            exit( 1 );
        }
        fBootDb = TRUE;
...
//              if (pRoomIndex->clan)
//              {
//                  bug("Load_rooms24: duplicate clan fields.",0);
//                  exit(1);
//              }
//              pRoomIndex->clan = clan_lookup(
                fread_string(fp);
          }
...
//              {
//                  bug("Load_rooms24: duplicate owner.",0);
//                  exit(1);
//              }
//
//              pRoomIndex->owner =
            ...
            {
                bug( "Load_rooms24: vnum %d has flag not 'DES'.", vnum );
                exit( 1 );
            }
        }
